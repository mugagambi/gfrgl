...
-- number of arguments.
  ...
  mkV : overload {
    mkV : (cry : Str) -> V ; -- regular, incl. cry-cries, Mers-Merses etc
...
-- hidden from the document.
  ...
  Cgender =  MorphoKik.Cgender ; 
  Number =  MorphoKik.Number ;
  Case   =  MorphoKik.NPCase ;
   a_wa  = G1 ;--%
   u_i   = G2 ;
  li_ya  = G3 ;
  ki_vi  = G4 ;
...
-- shortcut for regular relational nouns with "of".
 ...
 mkAV  v  = v ** { lock_AV = <>} ;
  ...
  iregA, regA : Str -> A = \s -> lin A (MorphoKik.regA s) ;
   cregA : Str -> A = \s -> lin A (MorphoKik.cregA s) ;
   --iregA : (fat,fatter : Str) -> A =\a,b -> lin A (MorphoKik.iregA a b);
  mkA = overload {
...
mkA2 = overload {
  ...
  regPN    : Str ->Cgender -> PN ;          
   nounPN : N -> PN ;
mkV = overload {
    mkV : (cry : Str) -> V=\v-> lin V (regV v ) ; -- regular, incl. cry-cries, Kiks-Merses etc
